...
### 3.1. **Диаграмма модуля анализа предпочтений и рекомендаций:**

```mermaid
graph LR
    A[Модуль анализа предпочтений] --> B[Анализ данных о пользователях]
    B --> C[Модуль рекомендаций]
    C --> D[Рекомендации для пользователя]
    A --> E[Система обработки пользовательских данных]
    C --> F[Исторические данные]
...
<details>
<summary>Диаграмма модуля анализа предпочтений и рекомендаций:</summary>

![Диаграмма модуля анализа предпочтений и рекомендаций](diagrams/preferences_analysis_diagram.svg)

</details>
- Описание:

- **Модуль анализа предпочтений** - занимается обработкой данных о пользователях и их интересах
- **Модуль рекомендаций** - использует данные из модуля анализа и генерирует рекомендации
- **Исторические данные** - могут быть дополнительным источником для улучшения рекомендаций
...
Для обеспечения стабильности и отказоустойчивости системы реализована централизованная обработка ошибок и исключений. В случае возникновения ошибок, система должна:

- Логировать ошибку в файл журнала или базу данных с указанием времени, типа ошибки и дополнительной информации, которая поможет в диагностике
- Возвращать пользователю информативные сообщения об ошибках, чтобы он мог понять, что произошло, и, при необходимости, предпринять действия (повторить запрос)
- Использовать подходы типа _retry_ (повторный запрос) для временных ошибок, таких как ошибки сети или API-сервера

Примеры ошибок:

#### 5.6.1. **Ошибка подключения к внешнему API**:

- В случае ошибки подключения, система будет пытаться подключиться повторно несколько раз с интервалом в 5 секунд. Если ошибка сохранится, должна возвратиться ошибка 503 ("Сервис временно недоступен")

<details>
<summary>Пример реализации повторных попыток с использованием `HttpClient`:</summary>
Если API не доступно, система должна автоматически делать повторные попытки с заданным интервалом или использовать кэшированные данные для выполнения запроса:
...
catch (HttpRequestException e)
{
    ...
    // Логика повторных попыток или использование кэшированных данных
}
catch (Exception e)
{
...
#### 5.6.2. **Ошибка валидации данных пользователя**:

- Если пользователь вводит некорректные данные для фильтрации контента, система вернет ошибку 400 с описанием проблемы

#### 5.6.3. **Перегрузка системы (например, слишком много одновременных запросов)**:

- Для защиты от перегрузки используется лимитирование запросов (rate limiting). В случае превышения лимита будет возвращен код ошибки 429 ("Слишком много запросов"), с указанием времени до следующей доступной попытки
...
| №   | Элемент             | Описание                              | Тип данных | Ограничения | Null or Not null | Unique | Пример заполнения               |
| --- | ------------------- | ------------------------------------- | ---------- | ----------- | ---------------- | ------ | ------------------------------- |
| 1   | user_id             | Уникальный идентификатор пользователя | INTEGER    | PK          | Not null         | Yes    | 1                               |
| 2   | google_email        | Email пользователя в Google           | STRING     | FK          | Not null         | Yes    | user@gmail.com                  |
| 3   | google_id           | Уникальный ID пользователя в Google   | STRING     |             | Not null         | Yes    | 1234567890                      |
| 4   | full_name           | Полное имя пользователя               | STRING     |             | Not null         | No     | John Doe                        |
| 5   | profile_picture_url | URL изображения профиля               | STRING     |             | Yes              | No     | https://example.com/profile.jpg |
| 6   | registered_at       | Дата регистрации пользователя         | TIMESTAMP  |             | Not null         | No     | 2025-03-25 10:00:00             |

---

#### Описание таблицы "Content"

| №   | Элемент      | Описание                                  | Тип данных | Ограничения | Null or Not null | Unique | Пример заполнения         |
| --- | ------------ | ----------------------------------------- | ---------- | ----------- | ---------------- | ------ | ------------------------- |
| 1   | content_id   | Уникальный идентификатор контента         | INTEGER    | PK          | Not null         | Yes    | 101                       |
| 2   | title        | Название контента                         | STRING     |             | Not null         | No     | "Inception"               |
| 3   | genre        | Жанр контента                             | STRING     |             | Not null         | No     | "Action"                  |
| 4   | content_type | Тип контента (например, фильм или сериал) | STRING     |             | Not null         | No     | "Movie"                   |
| 5   | release_date | Дата выпуска контента                     | DATE       |             | Not null         | No     | 2010-07-16                |
| 6   | description  | Описание контента                         | TEXT       |             | Yes              | No     | "A mind-bending thriller" |
| 7   | rating       | Рейтинг контента                          | DECIMAL    |             | Yes              | No     | 8.8                       |

#### Описание таблицы "User_preferences"

| №   | Элемент           | Описание                               | Тип данных | Ограничения | Null or Not null | Unique | Пример заполнения |
| --- | ----------------- | -------------------------------------- | ---------- | ----------- | ---------------- | ------ | ----------------- |
| 1   | preference_id     | Уникальный идентификатор предпочтения  | INTEGER    | PK          | Not null         | Yes    | 1                 |
| 2   | user_id           | Идентификатор пользователя             | INTEGER    | FK          | Not null         | No     | 1                 |
| 3   | preferred_genre   | Предпочитаемый жанр                    | STRING     |             | Yes              | No     | "Action"          |
| 4   | preference_weight | Важность предпочтения для пользователя | DECIMAL    |             | Yes              | No     | 0.8               |

#### Описание таблицы "User_activity"

| №   | Элемент       | Описание                                        | Тип данных | Ограничения | Null or Not null | Unique | Пример заполнения   |
| --- | ------------- | ----------------------------------------------- | ---------- | ----------- | ---------------- | ------ | ------------------- |
| 1   | activity_id   | Уникальный идентификатор активности             | INTEGER    | PK          | Not null         | Yes    | 1                   |
| 2   | user_id       | Идентификатор пользователя                      | INTEGER    | FK          | Not null         | No     | 1                   |
| 3   | content_id    | Идентификатор контента                          | INTEGER    | FK          | Not null         | No     | 101                 |
| 4   | activity_type | Тип активности (например, просмотрено, оценено) | STRING     |             | Not null         | No     | "watched"           |
| 5   | activity_date | Дата и время активности                         | TIMESTAMP  |             | Not null         | No     | 2025-03-25 10:00:00 |
...
- **Citus - расширение для PostgreSQL**, так как PostgreSQL не является изначально распределенной базой данных, у нее есть возможности репликации и расширения, позволяющие настроить ее для работы в распределенной среде с хорошей отказоустойчивостью и горизонтальной масштабируемостью, с использованием инструмента Citus. Citus - расширение для PostgreSQL, позволяющее масштабировать базу данных горизонтально. Citus разделяет данные по нескольким узлам, что позволяет эффективно распределять нагрузку. Благодаря этому PostgreSQL работает как распределенная база данных, обеспечивая поддержку шардирования, распределенных запросов и высокую производительность.
  Пример: Citus может разделить таблицу по ключу (например, идентификатору пользователя), и данные, соответствующие разным значениям ключа, могут храниться на разных узлах
- **PostgreSQL с BDR (Bi-Directional Replication)** - расширение, которое предоставляет двустороннюю репликацию, позволяя синхронизировать данные между несколькими серверами PostgreSQL
- **Использование распределенных файловых систем** - Для обеспечения хранения данных на нескольких машинах, можно использовать распределенные файловые системы, такие как Ceph, которые могут быть использованы для хранения данных PostgreSQL на нескольких узлах
- **PostgreSQL с кластеризацией** - PostgreSQL с использованием Patroni и etcd/Consul: Patroni — это инструмент для автоматического управления кластером PostgreSQL с высокой доступностью. Он использует системы распределенного хранилища (например, etcd или Consul) для хранения информации о текущем состоянии кластера и обеспечения автоматического переключения при отказах.
- **Horizontally Sharded PostgreSQL** - В некоторых случаях для распределения данных на несколько серверов (шардирование) используется горизонтальное масштабирование PostgreSQL с помощью сторонних инструментов.

Для обеспечения высокой доступности и отказоустойчивости, система будет использовать **репликацию** данных и подходы к распределению нагрузки:

- PostgreSQL поддерживает механизмы репликации (master-slave репликация), позволяющие создавать несколько копий базы данных для повышения отказоустойчивости и масштабируемости, при этом репликация может быть синхронной или асинхронной. Синхронная репликация - обновления на основном сервере не считаются успешными, пока они не будут переданы на все реплики. Асинхронная репликация - обновления на основном сервере считаются успешными сразу, без ожидания подтверждения от реплик
</details>
...
### 6.4. Кеширование данных

Для оптимизации производительности и снижения нагрузки на систему реализовано кеширование часто запрашиваемых данных.

Пример:

- **Кеширование рекомендаций**: Рекомендации для пользователя кешируются на уровне API, чтобы повторные запросы не генерировали дополнительные вычислительные расходы
- **Использование Redis**: Для кеширования данных, таких как предпочтения пользователей, данные о контенте и рекомендации, используется Redis, обеспечивающий быстрый доступ и снижение задержек при высоких нагрузках

### 6.5. Очереди для обработки запросов

Для управления нагрузкой в условиях большого количества пользователей используется **RabbitMQ**, что позволяетт асинхронно обрабатывать запросы, избегая перегрузки системы и увеличивая отказоустойчивость. (В случае перегрузки запросы ставятся в очередь и обрабатываются по мере доступности ресурсов).

### 6.6. Масштабируемость

Система построена с возможностью горизонтального масштабирования для обработки увеличивающегося числа пользователей. Модули обработки рекомендаций и API могут масштабироваться независимо для увеличения пропускной способности.
